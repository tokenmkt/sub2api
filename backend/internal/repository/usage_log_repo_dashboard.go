...
func (r *usageLogRepository) fillDashboardEntityStats(ctx context.Context, stats *DashboardStats, todayUTC, now time.Time) error {
	...
	accountStatsQuery := `
		SELECT
			...
			(
				SELECT COALESCE(SUM(
					CASE
						WHEN account_windows.used_fraction > 0 AND account_windows.current_cost > 0
				...
				FROM (
					...
					LEFT JOIN usage_logs ul
						...
						AND ul.created_at >= CASE
							WHEN a.platform = $5 AND a.session_window_start IS NOT NULL
								AND a.session_window_end IS NOT NULL AND NOW() < a.session_window_end
							THEN a.session_window_start
							WHEN a.platform = $8 THEN NOW() - INTERVAL '5 hours'
							ELSE date_trunc('hour', NOW())
						END
					WHERE a.deleted_at IS NULL AND a.status = $1 AND a.schedulable = true
						AND ((a.platform = $5 AND a.type IN ($6, $7)) OR (a.platform = $8 AND a.type = $6))
	...
	if err := scanSingleRow(
		...
		accountStatsQuery,
		[]any{
			service.StatusActive, service.StatusError, now, now,
			service.PlatformAnthropic, service.AccountTypeOAuth,
			service.AccountTypeSetupToken, service.PlatformOpenAI,
		},
		&stats.TotalAccounts,
