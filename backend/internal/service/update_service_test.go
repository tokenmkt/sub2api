...
func TestUpdateServiceListRollbackVersionsUsesConfiguredReleaseRepo(t *testing.T) {
	t.Setenv("SUB2API_UPDATE_REPO", "tokenmkt/sub2api")
	githubClient := &updateServiceGitHubClientStub{}
	svc := NewUpdateService(&updateServiceCacheStub{}, githubClient, "0.1.147", "release")

	_, err := svc.ListRollbackVersions(context.Background())

	require.NoError(t, err)
	require.Equal(t, "tokenmkt/sub2api", githubClient.recentRepo)
}
